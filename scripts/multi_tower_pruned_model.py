...
# ==========================================
# 1. FEATURE GROUP DEFINITIONS (PRUNED)
#    VV/VH lags and leads with near-zero attribution dropped.
# ==========================================
...
# ==========================================
# 3. MULTI-TOWER BiGRU MODEL (PRUNED DIMENSIONS)
# ==========================================

class MultiTowerBiGRU(nn.Module):
    """
    Pruned Multi-Tower BiGRU.
      Towers:
        RVI:  6 features -> hidden=32 (bidir 64)
        VV:   2 features -> hidden=16 (bidir 32)
        VH:   1 feature  -> hidden=16 (bidir 32)
        Wx:   3 features -> hidden=16 (bidir 32)
        Ctx:  6 features -> hidden=16 (bidir 32)
        Fusion Layer Input: 64 + 32 + 32 + 32 + 32 = 192-dim
    """
    def __init__(
        self,
...
def run_experiment(seq_len, df_train, df_test, output_dir):
    print(f"\n==================================================")
    print(f" TRAINING PRUNED MODEL: seq_length={seq_len}")
    print(f"==================================================")
    ...
    # Save outputs
    torch.save(model.state_dict(), os.path.join(output_dir, f"pruned_mt_bigru_seq{seq_len}.pt"))
    ...
    plt.title(f"Pruned Multi-Tower BiGRU (seq={seq_len}) — Training Curve")
    plt.legend(); plt.grid(True, linestyle=':', alpha=0.6); plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f"pruned_mt_training_curve_seq{seq_len}.png"), dpi=200); plt.close()
...
def main():
    ...
    joblib.dump(scaler, os.path.join(output_dir, "pruned_mt_scaler.pkl"))
    print("Pruned Scaler saved.")
    ...
    summary = {
        "architecture": "Pruned-MultiTowerBiGRU-5tower",
        "dropped_features": [
            "RVI_velocity_12", "VV_lag_6", "VV_lag_12", "VV_lead_6", "VV_lead_12",
            "VV_velocity_12", "raw_VH", "VH_lag_6", "VH_lag_12", "VH_lead_6",
            "VH_lead_12", "VH_velocity_12", "RVI_velocity_12"
        ],
        "retained_features": ALL_FEATURE_COLS,
        "results": results
    }
    with open(os.path.join(output_dir, "pruned_mt_metrics.json"), "w") as f:
