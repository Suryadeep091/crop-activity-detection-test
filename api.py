...
async def test_accuracy_by_geometry(request: GeometryRequest):
    ...
    try:
        ...
        pickle_url = upload_private_to_gcs(
            data=pickle_bytes, 
            destination_blob_name=f"no_lim_S2_analysis/{task_id}_full_2024.pkl", 
        ...
        report_url = upload_private_to_gcs(local_pdf_path, f"dummy_run_main/test_{task_id}.pdf", "application/pdf", is_file=True)
...
async def replay_test_from_pickle(task_id: str):
    try:
        ...
        pickle_bytes = download_from_gcs(f"no_lim_S2_analysis/{task_id}_full_2025.pkl")
        if not pickle_bytes:
        ...
        local_pdf_path = await generate_intelligence_report(full_data)
        report_url = upload_private_to_gcs(local_pdf_path, f"dummy_run_25/test_{task_id}_2025.pdf", "application/pdf", is_file=True)
