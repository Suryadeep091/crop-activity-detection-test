...
async def test_accuracy_by_geometry(request: GeometryRequest):
    ...
    try:
        ...
        report_url = upload_private_to_gcs(local_pdf_path, f"Raw_no_lim_S2_analysis/test_{task_id}_23-24.pdf", "application/pdf", is_file=True)
...
async def replay_test_from_pickle(task_id: str):
    try:
        ...
        local_pdf_path = await generate_intelligence_report(full_data)
        report_url = upload_private_to_gcs(local_pdf_path, f"dummy_run/test_{task_id}.pdf", "application/pdf", is_file=True)
