...
def impute_missing_ndvi_using_ensemble(df_all, df_dw_raw, coords, end_date_str, df_weather=None):
    ...
    # Preserve original sparse S1 observations BEFORE interpolation fills gaps to zeros
    ...
    # Use the sparse raw_RVI (before interpolation) to detect true S1 overpass dates
    ...
    if 'NDVI_predicted' in df_all_updated.columns:
        df_all_updated = df_all_updated.drop(columns=['NDVI_predicted'])
